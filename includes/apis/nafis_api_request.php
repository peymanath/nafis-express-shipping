...
function nafis_api_request($method, $endpoint, $token = null, $params = [], $body = null, $timeout = 10)
{
    ...
    if (is_wp_error($response)) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("❌ Nafis API Error ({$endpoint}): " . $response->get_error_message()); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
        }
    ...
    if (!isset($body['status']) || !$body['status']) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("⚠️ Nafis API response issue ({$endpoint}): " . wp_remote_retrieve_body($response)); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
        }
